...
- **Lifetime energy (SolarEdge One):** Per-optimizer lifetime energy comes from the energy-graph API (one GET per optimizer, 1-hour cache). When the cache is cold, `get_lifetime_energy_cached()` fetches these requests **in parallel** (thread pool, up to `MAX_PARALLEL_WORKERS` = 10 concurrent requests). **Site** lifetime can use the portal's dashboard production (Wh) from `get_dashboard_site_production_cached(installation_date)` when it is greater than the aggregated total; **inverter** and **string** can use the portal's layout/energy by-inverter (Wh) from `get_layout_energy_by_inverter_cached(installation_date)` when greater than their aggregated totals. The start date for these portal calls is the site **installation date** from site info (`get_site_info_cached()` → layout/information/site). Site info (installation date, peak power in kW) is cached per `SITE_INFO_CACHE_TTL` (2 h) and powers the site-level **Installation date** and **Peak power** sensors.
