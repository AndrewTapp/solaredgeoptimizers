...
- **Polling**: Lightweight “any new data?” check can use a batch request (up to `LIGHT_CHECK_BATCH_SIZE` = 5 optimizers) instead of one panel; 1-hour stale threshold for live values (legacy used 2 hours).
...
- **Status sensors:** Optimizer, string, and inverter status sensors show the device status from the API (e.g. "Active", "Inactive"). The icon changes dynamically based on the status value: check-circle for Active, alert-circle for Inactive, help-circle for unknown status. This makes it easy to spot inactive devices at a glance in the Home Assistant UI.
