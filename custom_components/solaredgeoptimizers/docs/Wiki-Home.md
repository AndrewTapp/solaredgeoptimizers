...
| Slow first load | Many optimizers → parallel requests for optimizer data and (when cache is cold) parallel lifetime-energy fetch; layout and lifetime energy cached after first run. Full refresh timeout 15 min. |
...
| TimeoutError on first load (many optimizers) | Full refresh has a 15-minute (900 s) timeout. Lifetime energy is now fetched in parallel when the cache is cold, so large sites (e.g. 50+ optimizers) should complete well within that time. If timeouts persist, check network or SolarEdge portal responsiveness. |
