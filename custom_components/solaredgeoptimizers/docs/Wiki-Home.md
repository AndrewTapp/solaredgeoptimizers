...
- **Aggregated** sensors at **string**, **inverter**, and **site** level: current (average), voltage (average), power, lifetime energy, last measurement, child counts (optimizer/string/inverter count), and status (strings and inverters only). At **site** level only (SolarEdge One): **Installation date** and **Peak power** (from portal layout/information/site).
...
| Site info cache (One only) | 2 hours | `SITE_INFO_CACHE_TTL`. Installation date and peak power from `get_site_info_cached()` (layout/information/site). Used for site-level sensors and as start date for portal lifetime calls. |
