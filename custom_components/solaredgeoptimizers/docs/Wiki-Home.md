...
- **Threshold**: When data is from **One API**, the coordinator uses `CHECK_TIME_DELTA_SOLAREDGE_ONE` = 1 hour; when from **Legacy API**, uses `CHECK_TIME_DELTA` = 2 hours (in `const.py`). The threshold follows the current data source (see **Obtained from** sensor). If sensors evaluate before the coordinator has set `_timetocheck`, they use the same One vs legacy delta based on `_has_batch_api` (not a hardcoded 2-hour fallback).  
...
- **Sessions / FD:** Thread-local sessions serve legacy CSRF paths; Basic Auth GETs cover layout/systemData. `close()` blocks new work and waits longer than `API_TIMEOUT_LONG` for tracked calls before closing sessions. Unload/removal shut down the coordinator before dual API close.
...
- **Levels**: `info` is reserved for setup/entity totals, API source changes, credential updates, one hierarchy-registration summary, and one successful dual-client close summary. `warning` reports actionable degradation such as CSRF/498 problems, a refresh with missing active measurements, a first per-serial lifetime 403, or a partial close failure. `debug` records configuration, polling decisions, cache activity, suffix resolution, bounded endpoint status/count summaries, and cleanup detail. Repeated lifetime 403s and individual empty-measurement cases are DEBUG.
- **Privacy and volume**: credentials, cookies, CSRF/token values, OAuth codes, response headers, complete API payloads, and raw query values are never logged. Per-sensor updates, per-optimizer aggregation, and per-node layout construction are intentionally omitted to keep large-site DEBUG logs usable.

**What debug logging covers:** config-flow decisions, setup/teardown sequencing, device-key and suffix resolution, adaptive light/full-refresh decisions, cache hit/miss and entry counts, portal lifetime override decisions, bounded HTTP status/size summaries, OAuth refresh/PKCE transitions without token values, CSRF bootstrap outcomes, and per-backend close detail. Enable with `logger: logs: solaredgeoptimizers: debug` in `configuration.yaml`.
...
| Optimizers not grouped under the correct string (duplicate strings at same position) | Optimizer registration uses `build_string_device_key_lookup()` to resolve the string's registry-scoped `via_device_id`, including duplicate/portal suffixes (e.g. `_str_1_0a`). Update and reload; re-add if hierarchy was created under an older build. |
| Deprecated `via_device` warning on Home Assistant 2026.8+ | Fixed in **v2.4.22**: parent registry entries are created first and child devices use `via_device_id`. The old API is removed in Home Assistant 2027.8. Reload after upgrading. |
...
| Repeating 403 on per-optimizer lifetime energy | Non-fatal (lifetime may stay unknown for that serial). From **v2.4.22** the first 403 per serial is WARNING; later ones are DEBUG. |
...
| Missing/invalid optimizer measurements | SolarEdge may retain replaced/inactive serials with empty live data. Individual cases are DEBUG and inactive devices are excluded from lightweight checks. A legacy full refresh emits one count-based WARNING only when active optimizers are missing measurements. Removing/re-adding does not remove portal entries. |
...
- **Unload**: Coordinator `async_shutdown` runs before platform teardown. The legacy and One clients then wait within a bounded interval for tracked work, and the dual API closes both backends in a `finally` path. It logs one INFO close summary, or WARNING if a backend fails.
- **Removing the integration**: Config-entry removal uses the same coordinator shutdown and API-close helpers before removing linked entities and devices. Delete from **Settings → Devices & services → Integrations** (not only HACS) so this cleanup runs.
...
| `string_device_keys_for_registration()` / `build_string_device_key_lookup()` | Compute string parent keys (with duplicate/portal suffixes) aligned with coordinator registration; used to resolve optimizer `via_device_id`. |
...
| `link_device_info()` | Identifiers-only `DeviceInfo` for entities; hierarchy is assigned once during device registration. |
