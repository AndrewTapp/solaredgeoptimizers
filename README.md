...
- **Optimizers**: Active (including blank status) optimizers come first (sorted by serial number), then other statuses. The first active optimizer keeps the original name; duplicates get suffixes.
...
- All API sessions and connections are closed when the integration is removed or reloaded: the legacy API tracks every session created (including those in the thread pool) and closes them on unload; the SolarEdge One API uses no persistent session. The dual API `close()` is called on unload and on config entry removal so file descriptors are released. This avoids file descriptor leaks and makes it safe to run for long periods.
